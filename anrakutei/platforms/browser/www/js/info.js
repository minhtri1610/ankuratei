    ...
    $scope.getInfo = function () {
        ...
        InfoService.getInfo(function (data) {
            ...
            Object.keys(data).forEach(function (key) {
                ...
                // \マーク表示設定（バックスラッシュをHTMLの特殊記号に置き換え）
                var title  = data[key].comment_title.replace(/\\/g, '&yen;');
                var body_s = data[key].short_comment.replace(/\\/g, '&yen;');
                var body   = data[key].comment_body.replace(/\\/g, '&yen;');
...
//                                    console.log("お知らせ件数：" + (i + 1) + "/" + len);
    ...
    $scope.removeInfoDir = function () {
        // ディレクトリエントリーを取得
        resolveLocalFileSystemURL(rootDir + infoPath, function (dirEntry) {
            // ディレクトリの再帰的削除
...
////////////////////////////////////////////////////////////////////////////////
    ...
    $scope.removeIconDir = function () {
        // ディレクトリエントリーを取得
        resolveLocalFileSystemURL(rootDir + iconPath, function (dirEntry) {
            // ディレクトリの再帰的削除
            dirEntry.removeRecursively(function () {
                // おしらせアイコン情報削除
                $scope.delInfoIcon();
