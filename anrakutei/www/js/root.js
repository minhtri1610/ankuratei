...
SlidingMenuCtrl = function ($scope, $timeout, MessageSV) {
        ...
        window.plugins.actionsheet.show(options, function (buttonIndex) {
            ...
            if (buttonIndex === 1) {
                ...
                if (navigator.connection.type === 'none') {
                    ...
                    // 安楽亭ホームページを表示
                    cordova.InAppBrowser.open(serverUrl + 'sp/', '_system', 'location=yes');
                }
            } else {
                window.plugins.actionsheet.hide();
            }
    ...
    $scope.directContact = function (mode) {
        ...
        var title = (mode === 1) ? '店舗に関するご意見・ご要望' : 'その他のお問い合わせ';
        var options = {
            'androidTheme': window.plugins.actionsheet.ANDROID_THEMES.THEME_HOLO_LIGHT,
            'title': title,
            'buttonLabels': ['ブラウザで開く'],
        ...
        window.plugins.actionsheet.show(options, function (buttonIndex) {
            ...
            if (buttonIndex === 1) {
                ...
                if (navigator.connection.type === 'none') {
                    ...
                    if (mode === 1) {
                        // 店舗へのお問い合わせ
                        cordova.InAppBrowser.open(serverUrl + 'sp/contact/index.html?type=5', '_system', 'location=yes');
                    } else {
                        // その他のお問い合わせ
                        cordova.InAppBrowser.open(serverUrl + 'sp/contact/form_ot.html?type=6', '_system', 'location=yes');
                    }
                }
            } else {
                window.plugins.actionsheet.hide();
    ...
    messageSV.alert = function (type) {
        ...
        switch (type) {
            ...
            case 1:
                // 接続エラー
                navigator.notification.alert(
                        '通信ができない環境の可能性があります。通信環境の良い場所でご利用ください。',
                        function (et) {
                            console.log(et);
                        },
                        '',
