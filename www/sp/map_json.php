...
$tenpocdlist=getShopLocation($conn,addslashes($_REQUEST["lon"]),addslashes($_REQUEST["lat"]));
		...
		*/
		while($row = pg_fetch_array($result)){
			...
			switch($row['egjkn2kbn']){
				case 1:
				$data .= " 金土・祝前日：{$row["fromtime2"]}～{$row["totime2"]}\n";
				break;
				case 2:
				$data .= " 金土：{$row["fromtime2"]}～{$row["totime2"]}\n";
				break;
				case 3:
				$data .= " 土日祝：{$row["fromtime2"]}～{$row["totime2"]}\n";
				break;
				case 4:
				$data .= " 金土日祝・祝前日：{$row["fromtime2"]}～{$row["totime2"]}\n";
				break;
				default:
				break;
			}
