<?php
    ...
    $date = split ('[/.-]', $_POST["txtDate"]);    // フォームタイプ
    $sltToi = $_POST["sltToi"];        // 1．お問い合わせ内容
    $txtTenpo = $_POST["hidTenpo"];    // 3. ご利用店舗名
    $hidTenpoCd = $_POST["hidTenpoCd"];// 3. ご利用店舗コード
    
    $txtIken = $_POST["txtIken"];      // 4．ご意見・ご要望
    $txtName = $_POST["txtName"];      // 5.お名前 
    $rdoSex = $_POST["rdoSex"];        // 6.性別
    $sltAge = $_POST["sltAge"];        // 7.年齢
    ...
    $txtAddr1 = $_POST["txtAddr1"];    // 8.住所(都道府県・市区郡町村) 
    $txtAddr2 = $_POST["txtAddr2"];    // 8.住所(○丁目○番地○号) 
    $txtAddr3 = $_POST["txtAddr3"];    // 8.住所(ビル名・マンション名)
    $txtTel = $_POST["txtTel"];        // 9.電話番号
    $txtMail = $_POST["txtMail"];      // 10.電子メールアドレス
    $txtYear = $date[0];      // 11.ご来店日(年)
    $txtMonth = $date[1];    // 11.ご来店日(月)
    $txtDay = $date[2];        // 11.ご来店日(日)
    $txtTime = $_POST["txtTime"];      // 11.ご来店時刻(時頃)
    $txtCount = $_POST["txtCount"];    // 11.ご来店日人数(人)
...
<form id="upload-form" name="USER_VOICE" method="post">
	...
	<dl>
		<dt>ご来店日時</dt>
		<dd>
			<input type=hidden name="txtYear" value="<?=$txtYear?>">
			<input type=hidden name="txtMonth" value="<?=$txtMonth?>">
			<input type=hidden name="txtDay" value="<?=$txtDay?>">
			<input type=hidden name="txtTime" value="<?=$txtTime?>">
			<input type=hidden name="txtCount" value="<?=$txtCount?>">
		...
		<dt>ご利用店舗名</dt>
		<dd class="required">
			<input type=hidden name="txtTenpo" value="<?=$txtTenpo?>">
			<input type=hidden name="hidTenpoCd" value="<?=$hidTenpoCd?>">
			<span style=font-size:16px;><?=$txtTenpo?>&nbsp;</span>
		</dd>
	...
	<div id="send_button">
		<input type="button" class="btnCheck" style="width: 140px;" name="btnSendMail" value="送信" onclick="funcSendMail3()">&nbsp;
		<input type="button" class="btnCheck" style="width: 140px;"                    value="戻る" onclick="self.history.back()">
	</div>
